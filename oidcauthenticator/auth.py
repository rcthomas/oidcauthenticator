...
"""
This authenticator was based off of the KeyCloakAuthenticator from
CERN (https://github.com/swan-cern).  It has been modified to work with Satosa.
...
class OIDCAuthenticator(GenericOAuthenticator):
    """OIDCAuthenticator based on upstream jupyterhub/oauthenticator"""
    ...
    async def authenticate(self, handler, data=None):
        ...
        try:
            self.log.info(user)
            decoded_token = self._decode_token(user['auth_state']['id_token'])
    ...
    async def _get_ssh_key(self, token, username):
        ...
        if self.skey != '':
            ...
            headers = httputil.HTTPHeaders(h)
    ...
    async def refresh_user(self, user, handler=None):
        ...
        try:
            ...
            # verifies it
            decoded_refresh_token = self._decode_token(auth_state['id_token'], options={"verify_signature": False})
            ...
            if diff_refresh < 0:
                ...
                access_token, refresh_token, id_token = self._refresh_token(auth_state['refresh_token'])
